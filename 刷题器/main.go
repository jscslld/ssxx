...
func main() {
	...
	mode[1][0]="[1]英雄篇";mode[1][1]="5f71e934bcdbf3a8c3ba51d5"
	mode[2][0]="[2]复兴篇";mode[2][1]="5f71e934bcdbf3a8c3ba51d6"
	mode[3][0]="[3]创新篇";mode[3][1]="5f71e934bcdbf3a8c3ba51d7"
	mode[4][0]="[4]信念篇";mode[4][1]="5f71e934bcdbf3a8c3ba51d8"
	//mode[5][0]="[5]限时赛";mode[5][1]="5f71e934bcdbf3a8c3ba51d9"
	//mode[6][0]="[6]抢十赛";mode[6][1]="5f71e934bcdbf3a8c3ba51da"
	ColorPrint("题目类型\n",FontColor.light_gray)
	for i:=1;i<=4;i++{
		ColorPrint(mode[i][0]+"\n",FontColor.light_gray)
	}
	ColorPrint("请输入您题目类型序号：",FontColor.light_gray)
	var mid int
	fmt.Scanln(&mid)
	_mid := mode[mid][1]
	//println(_mid)
	...
	ColorPrint("请输入循环次数：",FontColor.light_gray)
	fmt.Scanln(&times)
	ColorPrint("请输入答题间隔：",FontColor.light_gray)
	fmt.Scanln(&wait)
	for i1=1;i1<=times;i1++ {
		i2=0
		ColorPrint("[info] 现在开始第"+strconv.FormatInt(int64(i1),10)+"轮答题训练\n",FontColor.light_blue)
		...
		ColorPrint("[info] 答题结束，共回答正确：" + strconv.FormatInt(int64(ccount), 10) + "题，正确率",FontColor.light_gray)
		fmt.Printf("%.2f",float64(ccount)/float64(i2)*100)
		fmt.Println("%")
	}
	fmt.Println("按任意键退出...")
	var input string
